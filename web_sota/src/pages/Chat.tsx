...
const PERSONALITIES = [
	{
		...
		label: "Vienna Guide",
		prompt:
			"You are a Vienna local guide. Help with local tips, transit, Kaffeehaus culture, and Wienerisch charm.",
	},
	{
		id: "assistant",
		label: "Personal Assistant",
		prompt:
			"You are a personal assistant for Vienna life. Help with calendar, errands, and daily planning.",
	},
	{
		id: "analyst",
		label: "Urban Analyst",
		prompt:
			"You are an urban lifestyle analyst. Provide insights on Vienna neighborhoods, transit patterns, and city living.",
	},
...
export default function Chat() {
	...
	const [personality, setPersonality] = useState(
		() => localStorage.getItem(LS_PERSONALITY) || "vienna-guide",
	);
	...
	useEffect(() => {
		...
		apiGet<LlmStatus>(API.llmStatus)
			.then(setLlmStatus)
	...
	const handleExport = useCallback(() => {
		...
		a.href = url;
		a.download = `vilife-chat-${new Date().toISOString().slice(0, 10)}.txt`;
		a.click();
	...
	return (
		<div
			...
			className="flex flex-col h-[calc(100vh-10rem)] page-enter space-y-4"
		>
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					<select
						...
						className="bg-slate-800 text-sm text-slate-300 border border-slate-700 rounded px-2 py-1"
					>
						{PERSONALITIES.map((p) => (
					...
					{llmStatus && (
						<div className="flex items-center gap-1 text-sm font-black uppercase tracking-widest">
							<Cpu
								className={`w-4 h-4 ${llmStatus.ok ? "text-emerald-400" : "text-amber-400"}`}
							/>
							<span
								className={llmStatus.ok ? "text-emerald-400" : "text-amber-400"}
							>
								{llmStatus.ok
