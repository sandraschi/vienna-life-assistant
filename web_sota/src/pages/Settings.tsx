...
import { Settings as SettingsIcon, Loader2, Cpu, Save, RefreshCw } from 'lucide-react';
...
export default function Settings() {
    ...
    const statusHint = () => {
        ...
        if (status.ok) return <span className="text-emerald-400">Online · {status.model}</span>;
        if (llm?.provider === 'openai') {
            return <span className="text-amber-400">Set OpenAI API key and save</span>;
        }
        return (
            <span className="text-amber-400">
                Offline — start {llm?.provider === 'lmstudio' ? 'LM Studio (1234)' : 'Ollama (11434)'}
            </span>
        );
    };
    ...
    return (
        <div className="space-y-8 page-enter max-w-2xl">
            <div>
                ...
                <p className="text-slate-500 mt-2 text-sm">Switchable LLM · Ollama · LM Studio · OpenAI</p>
            </div>
            ...
            {!caps || !llm ? (
                ...
                <>
                    <div className="glass-card p-8 space-y-6">
                        ...
                        <label className="block text-[10px] font-black text-slate-600 uppercase tracking-widest">Provider</label>
                        <select
                        ...
                        {llm.provider === 'ollama' && (
                            <>
                                <input
                                    ...
                                    onChange={(e) => setLlm({ ...llm, ollama_url: e.target.value })}
                                />
                                <ModelSelect
                                    label="Ollama model"
                        ...
                        {llm.provider === 'lmstudio' && (
                            <>
                                <input
                                    ...
                                    onChange={(e) => setLlm({ ...llm, lmstudio_url: e.target.value })}
                                />
                                <ModelSelect
                        ...
                        {llm.provider === 'openai' && (
                            <>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setLlm({ ...llm, openai_base_url: e.target.value })}
                                />
                                <ModelSelect
                                    label="OpenAI model"
