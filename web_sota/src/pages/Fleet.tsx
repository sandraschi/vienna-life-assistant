...
export default function Fleet() {
    ...
    const load = async (withProbe: boolean) => {
        setLoading(true);
        try {
            const json = await apiGet<FleetOverview>(API.fleetOverview(withProbe ? 1 : 0));
            setData(json);
        } catch (e) {
    ...
    const filtered =
        filter === 'all' ? ships : ships.filter((s) => s.ship_class === filter || s.category === filter);
    ...
    return (
        <div className="space-y-10 page-enter">
            ...
            {data?.summary && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div className="glass-card p-6">
                        ...
                        <p className="text-3xl font-black text-white mt-2">{data.summary.total}</p>
                    </div>
                    <div className="glass-card p-6">
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Online</p>
                        <p className="text-3xl font-black text-emerald-400 mt-2">
                            {data.summary.probed ? data.summary.online ?? 0 : '—'}
                        </p>
                    </div>
                    <div className="glass-card p-6">
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Quarantined</p>
                        <p className="text-3xl font-black text-amber-400 mt-2">{data.summary.quarantined}</p>
                    </div>
                    <div className="glass-card p-6">
            ...
            <div className="flex flex-wrap gap-2 items-center">
                ...
                {['all', 'carrier', 'destroyer', 'frigate', 'Media', 'Creative', 'Robotics', 'Infra'].map((f) => (
                    <button
                        key={f}
                        type="button"
                        onClick={() => setFilter(f)}
            ...
            {loading && !data && (
                <p className="text-slate-500 text-sm uppercase tracking-widest font-bold">Loading fleet registry…</p>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filtered.map((ship) => {
                    const quarantined = ship.status === 'quarantined' || ship.status === 'archived';
                    return (
                        <div
                            key={ship.id}
                            className={`glass-card p-6 flex flex-col gap-4 ${
                                quarantined ? 'opacity-50 border-amber-500/20' : 'hover:border-white/20'
                            } transition-all`}
                        >
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-black text-white uppercase tracking-tight">{ship.name}</h3>
                                        <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">
                                            {CLASS_LABELS[ship.ship_class] ?? ship.ship_class} · {ship.category}
                                        </p>
                                    </div>
                                </div>
                                {data?.summary.probed && (
                                    <div className="flex items-center gap-2" title={ship.health}>
                                        <Circle className={`w-2 h-2 fill-current ${healthColor(ship.health)}`} />
                                    </div>
                                )}
                            </div>

                            <p className="text-[11px] text-slate-500 line-clamp-2 leading-relaxed">{ship.description}</p>

                            <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/[0.04]">
                                <span className="text-[9px] font-mono text-slate-600">
                                    {ship.frontend_port > 0 ? `:${ship.frontend_port}` : ship.backend_port > 0 ? `api :${ship.backend_port}` : 'no port'}
                                </span>
                                {quarantined ? (
                                    ...
                                    <span className="text-[9px] text-slate-600 uppercase">stdio only</span>
                                )}
                            </div>
                        </div>
                    );
            ...
            {data?.ops_root && (
                <p className="text-[9px] text-slate-700 font-mono">Registry: {data.ops_root}</p>
            )}
        </div>
    );
}
