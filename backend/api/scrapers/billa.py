...
Scrapes weekly offers from Billa Austria
"""
...
class BillaScraper:
    """Scraper for Billa.at weekly offers"""

    BASE_URL = "https://www.billa.at"
    ...
    async def scrape_offers(self) -> List[Dict[str, Any]]:
        """
        Scrape current weekly offers from Billa
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                offer_cards = soup.find_all(
                    "div", class_=["product-card", "offer-item"]
                )
                ...
                logger.info(f"Scraped {len(offers)} offers from Billa")
                return offers

        except Exception as e:
            logger.error(f"Failed to scrape Billa offers: {e}")
            return []
    ...
    def _parse_offer_card(self, card) -> Dict[str, Any]:
        ...
        product_name = (
            name_elem.get_text(strip=True) if name_elem else "Unknown Product"
        )
        ...
        try:
            price = (
                float("".join(filter(str.isdigit, price_text.replace(",", ".")))) / 100
            )
                ...
                original_price = (
                    float(
                        "".join(
                            filter(
                                str.isdigit,
                                original_price_elem.get_text().replace(",", "."),
                            )
                        )
                    )
                    / 100
        ...
        image_url = (
            img_elem.get("src") or img_elem.get("data-src") if img_elem else None
        )
    ...
    async def get_mock_offers(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "store": "billa",
                "product_name": "Billa Bio Wurst 200g",
            ...
            {
                "store": "billa",
                "product_name": "Manner Schnitten 75g",
            ...
            {
                "store": "billa",
                "product_name": "Billa Orangensaft 1L",
