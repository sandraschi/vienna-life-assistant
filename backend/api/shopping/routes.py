...
async def get_offers(
    ...
    today = date.today()
    query = query.filter(
        and_(StoreOffer.valid_from <= today, StoreOffer.valid_until >= today)
    )
    ...
    offers = (
        query.order_by(StoreOffer.discount_percentage.desc())
        .offset(offset)
        .limit(page_size)
        .all()
    )
    ...
    for store_name in stores_to_scrape:
        try:
            if store_name == Store.SPAR:
                scraper = SparScraper()
                offers = (
                    await scraper.get_mock_offers()
                    if use_mock
                    else await scraper.scrape_offers()
                )
            ...
            if not offers and not use_mock:
                logger.warning(
                    f"Real scraping failed for {store_name}, using mock data"
                )
                if store_name == Store.SPAR:
...
async def get_store_stats(db: Session = Depends(get_db)):
    ...
    best_discounts = (
        db.query(StoreOffer)
        .filter(and_(StoreOffer.valid_from <= today, StoreOffer.valid_until >= today))
